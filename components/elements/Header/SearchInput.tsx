...
import { getMeblByNameFx, searchMebelsFx } from '@/app/api/mebels'
...
const SearchInput = () => {
  ...
  const spinner = useStore(searchMebelsFx.pending)
  ...
  const handleSearchClick = async () => {
    ...
    getMebelAndRedirect(inputValue)
  }

  const searchMebel = async (search: string) => {
    try {
      setInputValue(search)
      const data = await searchMebelsFx({
        url: '/mebels/search',
        search,
      })

      const names = data
        .map((item: IMebel) => item.name)
  ...
  const getMebelAndRedirect = async (name: string) => {
    const mebel = await getMeblByNameFx({
      url: '/mebels/name',
      name,
    })
  ...
  const onSearchInputChange = (text: string) => {
    ...
    delayCallback(() => searchMebel(text))
  }
